...
def launcSnd():
    ...
    p= subprocess.Popen(['pasuspender','pd-extended', '2.1/src/soundengine/main.pd'], shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    return p


def launcBlender():
	p= subprocess.Popen(['wmctrl','-s 1'], shell=False,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
	p= subprocess.Popen(['/home/carlos/blender-2.63a-linux-glibc27-i686/blender','/home/carlos/audiogames/2.1/src/game2-auto.blend'], shell=False,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
	return p

def launcTracker():
	p= subprocess.Popen(['pd-extended','-noaudio', '2.1/src/tracking/tracker-norte.pd'], shell=False,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
	time.sleep(1)
	a= subprocess.Popen(['wmctrl','-s 1'], shell=False,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
	return p

def launcRouter():
	p= subprocess.Popen(['pd-extended','-noaudio', '2.1/src/tracking/routerOSC.pd'], shell=False,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
	return p
